...
export function MaxPlot(div, top, left, width, height, args) {
	...
	function guessRadius(coordCount) {
		...
		if (coordCount > 50000) return 0;
		else if (coordCount > 10000) return 2;
		else if (coordCount > 4000) return 4;
		else return 5;
	}
	...
	function createButton(
		...
		if (text !== null)
			if (text.length > 3) div.style['font-size'] = '11px';
			else div.style['font-size'] = '14px';
		...
		if (imgFname !== null && imgFname !== undefined) {
			...
			if (paddingTop !== null && paddingTop !== undefined) {
				...
				if (paddingBottom) img.style.paddingBottom = paddingBottom + 'px';
			}
			div.appendChild(img);
		}
		if (addSep === true) div.style['border-bottom'] = '1px solid #D7D7D7';
		if (addThickSep === true) div.style['border-bottom'] = '2px solid #C7C7C7';
	...
	function addZoomButtons(top, left, self) {
		...
		var fullDiv = createButton(
			width,
			height,
			'mpCtrlZoom100',
			'Zoom in. Keyboard: space',
			'100%',
			null,
			null,
			null,
	...
	function addModeButtons(top, left, self) {
		...
		var selectButton = createButton(
			...
			bSize,
			'mpIconModeSelect',
			'Select mode. Keyboard: shift or s',
			null,
			'mpimg/select.png',
			0,
			4,
		...
		var zoomButton = createButton(
			...
			bSize,
			'mpIconModeZoom',
			'Zoom-to-rectangle mode. Keyboard: Windows/Command or z',
			null,
			'mpimg/zoom.png',
			4,
			4,
	...
	function addProgressBars(top, left) {
		...
		for (var i = 0; i < 3; i++) {
			htmls.push(
				'<div id="mpProgressDiv' +
					i +
					'" style="display:none; height:17px; width:300px; background-color: rgba(180, 180, 180, 0.3)" style="">'
			);
			htmls.push(
				'<div id="mpProgress' + i + '" style="background-color:#666; height:17px; width:10%"></div>'
			);
			htmls.push(
				'<div id="mpProgressLabel' +
					i +
					'" style="color:white; line-height:17px; position:relative; top:' +
					i * 17 +
					'px;left:100px">Loading...</div>'
			);
			htmls.push('</div>');
		}
	...
	function scaleLines(lines, zoomRange, winWidth, winHeight) {
		...
		for (var lineIdx = 0; lineIdx < lines.length; lineIdx++) {
			...
			if (startInvis && endInvis) continue;
			if (startInvis) {
...
*/
	...
	function drawLabels(ctx, labelCoords, winWidth, winHeight, zoomFact) {
		...
		for (var i = 0; i < labelCoords.length; i++) {
			...
			if (coord === null) {
				// outside of view range, push a null to avoid messing up the order of bboxArr
				bboxArr.push(null);
				continue;
			}
	...
	this.setBackground = function (img) {
		/* */
		if (self.background === undefined || self.background === null) self.background = {};
		self.background.image = img;
		self.scaleBackground(self.background, self.port.initZoom, self.port.zoomRange);
		if (self.childPlot) self.childPlot.setBackground(img);
	};
	...
	this.scaleData = function () {
		...
		if (!self.coords)
			// window resize can call this before coordinates are loaded.
			return;

		var borderMargin = self.port.radius;
		self.calcRadius();

		self.coords.px = scaleCoords(
			self.coords.orig,
			borderMargin,
			self.port.zoomRange,
			self.canvas.width,
		...
		if (self.coords.lines)
			self.coords.pxLines = scaleLines(
				self.coords.lines,
				self.port.zoomRange,
				self.canvas.width,
				self.canvas.height
	...
	this.setSize = function (width, height, doRedraw) {
		...
		if (width === null) width = self.div.getBoundingClientRect().width;

		self.quickResize(width, height);

		if (self.coords) self.scaleData();

		if (doRedraw === undefined || doRedraw === true) self.drawDots();
	};
	...
	this.setCoords = function (coords, clusterLabels, coordInfo, opts) {
		...
		self.scalingDone = false;
		if (coords.length === 0) alert('cbDraw-setCoords called with no coordinates');
		...
		if (oldRadius) self.port.initRadius = oldRadius;
		if (oldAlpha) self.port.initAlpha = oldAlpha;
		self.coords = {};
		...
		for (var i = 0; i < coords.length / 2; i++) {
			...
			if (!isHidden(cellX, cellY)) count++;
		}

		if (opts.lines) self._setLines(opts['lines'], opts);
		self.scaleData();
	};
	...
	this.calcRadius = function () {
		...
		if (baseRadius === 0) baseRadius = 0.7;
	...
	this.drawDots = function () {
		...
		if (alpha === undefined) alert('internal error: alpha is not defined');
		if (coords === null) alert('internal error: cannot draw if coordinates are not set yet');
		if (colArr.length !== coords.length >> 1) {
			alert(
				'internal error: cbDraw.drawDots - colorArr is not 1/2 of coords array. Got ' +
					colArr.length +
					' color values but coordinates for ' +
					coords.length / 2 +
					' cells.'
			);
		}
	...
	this.cellsInRect = function (x1, y1, x2, y2) {
		...
		for (var i = 0; i < pxCoords.length / 2; i++) {
			var cellX = pxCoords[i * 2];
			var cellY = pxCoords[i * 2 + 1];
			if (cellX >= x1 && cellX <= x2 && cellY >= y1 && cellY <= y2) res.push(i);
		}
		return res;
	};
	...
	this.zoomBy = function (zoomFact, xPx, yPx) {
		...
		if (newZoom < 0.01 || newZoom > 1500) return zr;
	...
	this.selectClear = function (skipNotify) {
		/* clear selection */
		self.selCells.clear();
		if (self.onSelChange !== null && skipNotify !== true) self.onSelChange(self.selCells);
	};
	...
	this.selectSet = function (cellIds) {
		...
		for (let i = 0; i < cellIds.length; i++) selCells.add(cellIds[i]);
		self._selUpdate();
	};
	...
	this._selUpdate = function () {
		...
		if (self.onSelChange !== null) self.onSelChange(self.selCells);
	};
	...
	this.drawMarquee = function (x1, y1, x2, y2, forceAspect) {
		...
		} else selectHeight = Math.abs(y1 - y2);
	...
	this.activatePlot = function () {
		...
		if (self.parentPlot === null) return false;
	...
	this.onMouseMove = function (ev) {
		...
		if (self.coords.labelBbox !== null && self.mouseDownX === null) {
			...
			if (labelInfo === null) {
				...
				if (self.onLabelHover !== null) self.onLabelHover(labelInfo[0], labelInfo[1], ev);
			}
		}
		...
		if (self.mouseDownX !== null) {
			...
			if ((ev.altKey || self.dragMode === 'move') && self.panCopy !== null) {
				...
				if ((self.dragMode === 'zoom' && !anyKey) || ev.metaKey) forceAspect = true;
	...
	this.onNoMouseMove = function () {
		...
		if (cellIds === null && self.onNoCellHover !== null && self.selCells === null)
			self.onNoCellHover();
		else if (self.onCellHover !== null) self.onCellHover(cellIds);
	};

	this.onMouseDown = function (ev) {
		/* user clicks onto canvas */
		if (self.activatePlot()) return; // ignore the first click into the plot, if it was the activating click
		let xCanvas = ev.clientX - self.canvas.getBoundingClientRect().left;
		let yCanvas = ev.clientY - self.canvas.getBoundingClientRect().top;
	...
	this.onMouseUp = function (ev) {
		...
		// marquee select
			...
			if (!ev.shiftKey) self.selectClear(true);
	...
	this.onWheel = function (ev) {
		...
		if (self.parentPlot !== null) return;
		var normWheel = normalizeWheel(ev);
		let xCanvas = ev.clientX - self.canvas.getBoundingClientRect().left;
		let yCanvas = ev.clientY - self.canvas.getBoundingClientRect().top;
		var spinFact = 0.1;
		if (ev.ctrlKey)
			// = OSX pinch and zoom gesture (and no other OS/mouse combination?)
	...
	this.getLabels = function () {
		...
		for (var i = 0; i < labels.length; i++) ret.push(labels[i][2]);
		return ret;
	};
	...
	this.setLabels = function (newLabels) {
		...
		for (var i = 0; i < newLabels.length; i++) self.coords.labels[i][2] = newLabels[i];
	...
	this._setLines = function (lines, attrs) {
		...
		if (!attrs) self.coords.lineAttrs = {};
		else self.coords.lineAttrs = attrs;
	};

	this.activateMode = function (modeName) {
		/* switch to one of the mouse drag modes: zoom, select or move */
		if (modeName === 'zoom') self.prevMode = self.dragMode;
		else self.prevMode = null;

		self.dragMode = modeName;

		var cursor = null;

		if (modeName === 'move') cursor = 'all-scroll';
		else if (modeName === 'zoom') cursor = 'zoom-in';
		else if (modeName == 'select') cursor = 'crosshair';
		//else
		//cursor= 'default';
		...
		if (self.childPlot) self.childPlot.activateMode(modeName);
	};
	...
	this.randomDots = function (n, radius, mode) {
		...
		if (mode !== undefined) self.mode = mode;
		self.port.radius = radius;
	...
	this.split = function () {
		...
		if (self.background) plot2.setBackground(self.background.img);
	...
	this.getWidth = function () {
		...
		if (self.childPlot) return self.width + self.childPlot.width;
